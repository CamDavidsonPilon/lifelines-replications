...
class CRCSplineFitter(SplineFitterMixin, ParametricRegressionFitter):
    ...
    def _cumulative_hazard(self, params, T, Xs):
        ...
        for i in range(2, self.n_baseline_knots):
            H *= safe_exp(params["gamma%d_" % i] * self.basis(logT + Xbeta, self.knots[i-1], min_knot=self.knots[0], max_knot=self.knots[-1]))
        return H
...
def generate_data(n=1000):
    ...
    def S(t, x):
        return p*np.exp(-lambda1 * (t ** gamma1) * np.exp(-x * beta * gamma1)) + (1-p) * np.exp(-lambda2 * (t ** gamma2) * np.exp(-x * beta * gamma2))
